...
class PromptLearner(nn.Module):
    def __init__(self, model, tokenizer, classnames, n_ctx=8, ctx_init="a photo of a", device="cuda"):
        super().__init__()
        self.model = model
...
def main(args):
    ...
    train_ds = get_transfer_dataset(args.dataset, args.data_root, args.train_split, preprocess_train)
    val_ds   = get_transfer_dataset(args.dataset, args.data_root, args.val_split, preprocess_val)
    test_ds  = get_transfer_dataset(args.dataset, args.data_root, args.test_split, preprocess_val)
    ...
    if args.optimizer_name == "Adam":
        ...
        optimizer = SAM(model.parameters(), base_optimizer, rho=args.sam_rho, **opt_kwargs)
    elif args.optimizer_name == "ASAM":
        optimizer = SAM(model.parameters(), base_optimizer, adaptive=True, rho=args.sam_rho, **opt_kwargs)
    else:
    ...
    while step < args.max_steps:
        ...
        if args.test_every > 0 and (step % args.test_every == 0 or step == 1):
            model.eval()
            val_acc = evaluate_prompt_classifier(model, prompt_learner, val_loader, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, required=True,
                    choices=["flowers102", "cars", "food101"])
    parser.add_argument("--data-root", type=str, default="./data/transfer")
    parser.add_argument("--train-split", type=str, default="train")
    parser.add_argument("--val-split", type=str, default="val")  # for cars/food you can map val->test
    parser.add_argument("--test-split", type=str, default="test")

    # prompt learning
    parser.add_argument("--n-ctx", type=int, default=8, help="Number of learnable context tokens.")
    parser.add_argument("--ctx-init", type=str, default="a photo of a", help="Text to initialize context from.")
    parser.add_argument("--freeze-logit-scale", action="store_true")
    parser.add_argument("--prompt-lr", type=float, default=None, help="If set, overrides learning-rate for prompt params only.")


    # paths to data
    parser.add_argument("--coco-train-images", type=str, default="./data/coco/images/train2017",
                        help="COCO train2017 image directory (contains JPEGs).")
    parser.add_argument("--coco-train-ann", type=str, default="./data/coco/annotations/captions_train2017.json",
    ...
    parser.add_argument("--imagenet-gt", type=str, default="./data/imagenet/ILSVRC2012_validation_ground_truth.txt", help="ILSVRC2012 validation ground-truth labels file (50k lines, 1..1000).",)
    parser.add_argument("--imagenet-classnames", type=str, default="./data/imagenet/ilsvrc2012_classnames_1k.txt", help="Devkit-derived class names in ILSVRC2012 index order (1000 lines).",)
    ...
    # optimizer
    parser.add_argument("--optimizer-name", type=str, required=True, choices=["Adam", "SAM", "ASAM", "AdamSAM"])
    parser.add_argument("--learning-rate", type=float, default=1e-5)
    parser.add_argument("--weight-decay", type=float, default=0.0)
    parser.add_argument("--sam-rho", type=float, default=0.05)

    # training
    parser.add_argument("--max-steps", type=int, default=20000, help="Total number of optimizer steps to run (step-based training budget).",)
    parser.add_argument("--warmup-steps", type=int, default=500, help="Number of linear warmup steps.")
    parser.add_argument("--warmup-start-factor", type=float, default=1e-3, help="Warmup start factor for LinearLR (lr starts at start_factor * base_lr).",)
