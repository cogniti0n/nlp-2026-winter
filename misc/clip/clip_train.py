...
def sam_step(
    ...
    with torch.cuda.amp.autocast(enabled=use_autocast, dtype=autocast_dtype):
        image_features, text_features, logit_scale = forward_features(
    ...
    optimizer.zero_grad(set_to_none=True)
    with torch.cuda.amp.autocast(enabled=use_autocast, dtype=autocast_dtype):
        image_features_2, text_features_2, logit_scale_2 = forward_features(
            model, images, text_tokens
        )
...
def evaluate_zeroshot_imagenet(model, classifier, val_loader, device):
    ...
    if (
        classifier.shape[0] != model.text_projection.shape[1]
        and classifier.shape[1] == model.text_projection.shape[1]
    ):
